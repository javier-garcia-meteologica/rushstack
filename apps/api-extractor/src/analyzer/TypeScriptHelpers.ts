...
export class TypeScriptHelpers {
  ...
  public static getModuleSpecifier(nodeWithModuleSpecifier: ts.ImportDeclaration
    | ts.ExportDeclaration | ts.ImportTypeNode): string | undefined {

    if (nodeWithModuleSpecifier.kind === ts.SyntaxKind.ImportType) {
      return ((nodeWithModuleSpecifier.argument as ts.LiteralTypeNode).literal as ts.StringLiteral).text.trim();
    }
