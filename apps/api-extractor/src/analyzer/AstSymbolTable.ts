...
export class AstSymbolTable {
  ...
  // for that node.
  private readonly _entitiesByNode: Map<ts.Identifier | ts.ImportTypeNode, AstEntity | undefined>
  ...
  private _fetchEntityForNode(node: ts.Identifier | ts.ImportTypeNode,
    ...
    if (!referencedAstEntity) {
      if (node.kind === ts.SyntaxKind.ImportType) {
        referencedAstEntity = this._exportAnalyzer.fetchReferencedAstEntityFromImportTypeNode(node,
          governingAstDeclaration.astSymbol.isExternal);
      } else {
  ...
  public findPreferredImportAlternative(astImport: AstImport) {
    return this._exportAnalyzer.findPreferredImportAlternative(astImport);
  }
}
